...
class RaportModel extends Model{
	...
	public function download() {
		...
		if($post['submit']){
			...
			if($post['event']==='hours'){
				...
				$this->query("SELECT evt_salto_ts AS data, evt_salto_user_name AS uzytkownik, evd.evd_dsc AS zdarzenie, REPLACE(REPLACE((SELECT 'Budynek')::text || ' ' || REPLACE(evt_salto_door_name,'/',' drzwi '), 'Budynek Bramka', 'Bramka'), 'Budynek Liczba', 'Liczba') AS drzwi, evt_ack.ack_cmt AS komentarz FROM zewng.evt JOIN zewng.evt_ack ON evt_id = ack_evt_id JOIN zewng.evd ON evt.evt_evd_id = evd.evd_id WHERE evt_salto_user_name ilike '$raport_username' AND evt_salto_door_name ilike '$raport_doorname' AND evt_evd_id IN (select evd_id from zewng.evd WHERE evd_isalarm IS $event) AND evt_date >= :start AND evt_date <= :end ORDER BY evt_salto_ts");
			}else{
				$this->query("SELECT evt_salto_ts AS data, evt_salto_user_name AS uzytkownik, evd.evd_dsc AS zdarzenie, REPLACE(REPLACE((SELECT 'Budynek')::text || ' ' || REPLACE(evt_salto_door_name,'/',' drzwi '), 'Budynek Bramka', 'Bramka'), 'Budynek Liczba', 'Liczba') AS drzwi FROM zewng.evt JOIN zewng.evd ON evt.evt_evd_id = evd.evd_id WHERE evt_salto_user_name ilike '$raport_username' AND evt_salto_door_name ilike '$raport_doorname' AND evt_evd_id IN (select evd_id from zewng.evd WHERE evd_isalarm IS $event) AND evt_date >= :start AND evt_date <= :end ORDER BY evt_salto_ts");
			}
			$this->bind(':start', $post['start']);
			$this->bind(':end', $post['end']);
			$rows = $this->resultSet();
			if($rows) {
				Messages::setMsg('Raport gotowy do pobrania', 'success');
				return $rows;
			}
			return;
		}
	...
	public function people() {
		$this->query("SELECT DISTINCT evt_salto_user_name FROM zewng.evt WHERE evt_salto_user_type = 0 AND evt_salto_ts >= '2019-09-04' AND evt_salto_user_name != '' ORDER BY evt_salto_user_name");
		$rows = $this->resultSet();
		$result=array();
